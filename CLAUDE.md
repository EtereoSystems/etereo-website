...
**Two easings, on purpose.** `--rv-ease` is the site's arrival curve and `.reveal`, the project panel and the view transition all read it (see the `--rv-*` note in `global.css`). The mobile menu reads `--menu-ease` instead. `--rv-ease` is a hard ease-out that spends 87% of its distance in the first third of the duration and creeps through the rest — exactly right for something that should already be settled by the time you scroll to it, and a snap for a panel you are watching open, however long you make the duration. Lengthening one without changing the other just adds a tail. If a new interaction is something the user is *watching*, it wants `--menu-ease`; if it is something that should be finished when they arrive, it wants `--rv-ease`.

The menu also opens and closes at different speeds — 440 ms out, 280 ms back — because a close happens while the page is already moving to wherever the tapped link goes. The mechanism is easy to misread: **a rule's own `transition` is what runs when its class is removed**, so the short durations sit on `.nav__links` and the long ones on `.nav__links.open`. Changing the base rule changes the close, not the open.
...
- **`prefers-reduced-motion` covers every autonomous animation.** Lenis is skipped in `App.tsx`, `.reveal` is neutralised in `global.css`, the tech marquee and the `panelIn` panel entrance stop in `components.css`, and `Macbook.tsx` multiplies its idle `Math.sin(t * …)` float/sway by an `idle` factor read from `motionPref` in [src/store.ts](src/store.ts) (a plain mutable object like `heroScroll`, so the frame loop can read it without re-rendering). Deliberately still moving: the scroll-driven choreography itself (that is the user's own input, and it carries the content), `.card`/`.btn` hover transforms, and the mobile menu's own open/close (they answer a user action). The menu's per-item stagger is decoration and does stop — its override is nested inside the `max-width: 720px` block rather than hoisted to a top-level one, because it has to beat the `transition-delay` set a few lines above it and a media query carries no specificity of its own, so only source order decides.

  The mobile menu panel also carries `visibility: hidden` while shut, with the visibility transition delayed by the fade so closing still animates. Before that it was moved off-screen with `translateY(-140%)`, which hides an element from view but leaves it in the tab order — a keyboard user at mobile width could tab into an invisible menu.
